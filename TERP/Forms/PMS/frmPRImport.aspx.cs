...
{
    public partial class frmPRImport : System.Web.UI.Page
    {
        TERPSession tERPSession;
        int row_added = 0;
        string prefix = "PRImport";
        ...
        private string ValidateField(string fields)
        {
            ...
            if (tmp[0].Trim() == "")
            {
                return "PR No can not be empty.";
            }

            if (tmp[0].Trim().Length > 8)
            {
                return "PR No is too long.";
            }
                ...
                if (content != null)
                {
                    try
                    {
                        ...
                        for (int i = 1; i < content.Count; i++)
                        {
                            ...
                            dr["pr_no"] = fields[0].ToString().Trim();
                            ...
                            dr["description"] = fields[13].ToString().Trim();
                        ...
                        //empty temp table
                        dsTemp.DeleteCommand = "DELETE FROM PMS_master_pr_temp WHERE upload_by='" + tERPSession.User_id + "'";
                        dsTemp.Delete();

                        //insert into temp table to calculate
                        DbHelper.insertBulkIntoDatabaseWithConnectionString(dsTemp.ConnectionString, dt, "PMS_master_pr_temp", field);

                        //Check Approval Code active
                        dt = DbHelper.getDataTable("SELECT approval_code FROM PMS_master_pr_temp INNER JOIN PMS_approval ON " +
                            "PMS_master_pr_temp.approval_code=PMS_approval.approval_code AND PMS_approval.status='Inactive' WHERE PMS_master_pr_temp.upload_by='" + tERPSession.User_id + "'", dsTemp.ConnectionString);
                        if (dt != null)
                        {
                        ...
                        //Check valid email
                        dt = DbHelper.getDataTable("SELECT request_by FROM PMS_master_pr_temp WHERE upload_by='" + tERPSession.User_id + "' AND RTRIM(LTRIM(request_by))=''",dsTemp.ConnectionString);
                        if (dt==null)
                        {
                        ...
                        dt = DbHelper.getDataTable("SELECT request_by FROM PMS_master_pr_temp WHERE NOT EXISTS (SELECT DISTINCT user_email FROM TERP_users) AND upload_by='" + tERPSession.User_id + "'", dsTemp.ConnectionString);
                        ...
                        //Send Email
                        dt = DbHelper.getDataTable("SELECT DISTINCT pr_no,user_full_name,user_email,user_id FROM vPMS_pr_approver_importing WHERE upload_by='" + tERPSession.User_id + "'", dsTemp.ConnectionString);
                        if (dt!=null && dt.Rows.Count>0)
                        {
                            foreach (DataRow row in dt.Rows)
                            {
                                string body = "Dear " + row.ItemArray[1].ToString().Trim() + "<br/><br/>\n";
                                body += "You have a Purchase Requisition <a href='http://192.168.1.142/terp/Forms/PMS/frmPRApproval.aspx?pr_no=" + row.ItemArray[0].ToString().Trim() + "'>[" + row.ItemArray[0].ToString().Trim() + "]</a> need approved." + "<br/><br/>\n";
                        ...
                        dt = DbHelper.getDataTable("SELECT * FROM vPMS_master_pr_new_upload", dsTemp.ConnectionString);
                        if (dt != null) row_added = dt.Rows.Count;

                        if (row_added>0)
                        {
                            //add new row from temp table
                            dsTemp.InsertCommand = "INSERT INTO PMS_master_pr (" + field + ") SELECT " + field + " FROM vPMS_master_pr_new_upload";
                            dsTemp.Insert();
                        }

                        //row_added = 0;
                        ////get new record from temp table
                        //dt = DbHelper.getDataTable("SELECT a.pr_no,a.line,a.item_number FROM PMS_master_pr a WHERE EXISTS (SELECT b.pr_no, b.line, b.item_number FROM PMS_master_pr_temp b WHERE upload_by='" + tERPSession.User_id + "' and a.pr_no = b.pr_no AND a.line = b.line AND a.item_number = b.item_number)", dsTemp.ConnectionString);
                        //if (dt != null && dt.Rows.Count > 0)
                        //{
                        ...
                        //Remove moved rows
                        dsTemp.DeleteCommand = "DELETE FROM PMS_master_pr_temp WHERE pr_no IN (SELECT DISTINCT pr_no FROM PMS_master_pr WHERE upload_by='" + tERPSession.User_id + "')";
                        dsTemp.Delete();
